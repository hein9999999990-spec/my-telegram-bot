...
# --- Settings ---
TOKEN = "8453592254:AAGrBzRmSLC6ZNrBbqVkgmuk9NHZ6vZgzoM"
ADMIN_ID = "8176057500"
...
def check_join(user_id):
    for channel in MUST_JOIN:
        try:
            ...
            # Channel ထဲမှာ မရှိရင် ဒါမှမဟုတ် Error တက်ရင် False ပြန်မယ်
            return False
    return True

def show_menu(message):
    # message က text message လား callback လား ခွဲခြားဖို့
...
def history(message):
    ...
    # SyntaxError မဖြစ်အောင် string formatting ကို သေချာပြန်ပြင်ထားပါတယ်
    if h:
...
def broadcast(message):
    if str(message.from_user.id) == ADMIN_ID:
        ...
        count = 0
        # RuntimeError မဖြစ်အောင် list(users.keys()) ကို သုံးထားပါတယ်
